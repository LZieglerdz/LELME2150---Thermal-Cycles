...
for gr in GROUPS:
    try:
        ...
        try:
            ...
            (s_1g,s_2g,s_3g,s_4g,s_1,s_2,s_3,s_4,s_5,s_6,s_7,s_8,s_8p,s_8pp,s_9,s_9p,s_9pp,s_10p,s_10pp) = s
    ...
    data    = [ ['1g    ','%.2f'%(T[0]-273.15), '%.1f'%(p[0]*1e-3), '%.2f'%(x[0]), '%.1f'%(h[0]*1e-3), '%.3f'%(s[0]*1e-3), '%.1f'%(e[0]*1e-3)],
                ...
                ['1    ','%.2f'%(T[4]-273.15), '%.1f'%(p[4]*1e-3), '%.2f'%(x[4]), '%.1f'%(h[4]*1e-3), '%.3f'%(s[4]*1e-3), '%.1f'%(e[4]*1e-3)],
                ['2    ','%.2f'%(T[5]-273.15), '%.1f'%(p[5]*1e-3), '%.2f'%(x[5]), '%.1f'%(h[5]*1e-3), '%.3f'%(s[5]*1e-3), '%.1f'%(e[5]*1e-3)],
                ['3    ','%.2f'%(T[6]-273.15), '%.1f'%(p[6]*1e-3), '%.2f'%(x[6]), '%.1f'%(h[6]*1e-3), '%.3f'%(s[6]*1e-3), '%.1f'%(e[6]*1e-3)],
                ['4    ','%.2f'%(T[7]-273.15), '%.1f'%(p[7]*1e-3), '%.2f'%(x[7]), '%.1f'%(h[7]*1e-3), '%.3f'%(s[7]*1e-3), '%.1f'%(e[7]*1e-3)],
                ['5    ','%.2f'%(T[8]-273.15), '%.1f'%(p[8]*1e-3), '%.2f'%(x[8]), '%.1f'%(h[8]*1e-3), '%.3f'%(s[8]*1e-3), '%.1f'%(e[8]*1e-3)],
                ['6    ','%.2f'%(T[9]-273.15), '%.1f'%(p[9]*1e-3), '%.2f'%(x[9]), '%.1f'%(h[9]*1e-3), '%.3f'%(s[9]*1e-3), '%.1f'%(e[9]*1e-3)],
                ['7    ','%.2f'%(T[10]-273.15), '%.1f'%(p[10]*1e-3), '%.2f'%(x[10]), '%.1f'%(h[10]*1e-3), '%.3f'%(s[10]*1e-3), '%.1f'%(e[10]*1e-3)],
                ['8    ','%.2f'%(T[11]-273.15), '%.1f'%(p[11]*1e-3), '%.2f'%(x[11]), '%.1f'%(h[11]*1e-3), '%.3f'%(s[11]*1e-3), '%.1f'%(e[11]*1e-3)],
                ...
                ['9    ','%.2f'%(T[14]-273.15), '%.1f'%(p[14]*1e-3), '%.2f'%(x[14]), '%.1f'%(h[14]*1e-3), '%.3f'%(s[14]*1e-3), '%.1f'%(e[14]*1e-3)],
